    ...
    <div>
      <section className="relative bg-navy-radial text-cream py-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative text-center">
          <p className="font-accent italic text-gold-300 text-xl mb-2">Experiences</p>
          <h1 className="text-5xl font-bold mb-4">Moments That Stay Forever</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex gap-2 overflow-x-auto scrollbar-hide">
          {TAGS.map(t => (
            <button key={t} onClick={() => setActive(t)}
              className={`px-4 py-1.5 rounded-full text-xs font-semibold border whitespace-nowrap transition flex-shrink-0 ${
                active === t ? 'bg-gold-500 text-navy-900 border-gold-500' : 'border-navy-200 text-navy-600 hover:border-gold-400 hover:text-gold-600'
              }`}>{t}</button>
          ))}
        </div>
      </section>
      ...
      <section className="py-12 bg-cream">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filtered.map(e => (
              <Card key={e.title} hover className="flex flex-col">
